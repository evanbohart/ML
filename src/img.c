...
int main(void)
{
    ...
    nn_add_layer(&net, conv_layer_alloc(8, 8, 64, BATCH_SIZE, 3, 64, 1, same, RELU));
    nn_add_layer(&net, maxpool_layer_alloc(8, 8, 64, BATCH_SIZE, 2));
    nn_add_layer(&net, conv_dropout_layer_alloc(4, 4, 64, BATCH_SIZE, 0.2));
    nn_add_layer(&net, conv_layer_alloc(4, 4, 64, BATCH_SIZE, 3, 128, 1, same, RELU));
    nn_add_layer(&net, maxpool_layer_alloc(4, 4, 128, BATCH_SIZE, 2));
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        for (int j = 0; j < 10 * 1000 / BATCH_SIZE; ++j) {
            ...
            mat_softmax(*predicted, *predicted);
